...
export function exportMarkdown(
  ...
  return [
    ...
    ...(run.topicBatch
      ? [
          `## Topic Batch Report`,
          `- Summary: ${run.topicBatch.summary}`,
          '',
          `### Workflow Artifact Analysis`,
          ...run.topicBatch.aggregateWorkflowReviews.map((item) => `- **${item.label} ${item.score}**: ${item.note}`),
          '',
          `### Final Output Analysis`,
          ...run.topicBatch.aggregateFinalReviews.map((item) => `- **${item.label} ${item.score}**: ${item.note}`),
          '',
          `### Process Optimizations`,
          ...run.topicBatch.processOptimizations.map((item) => `- ${item}`),
          '',
          `### Topic Items`,
          ...run.topicBatch.items.flatMap((item, index) => [
            `#### ${index + 1}. ${item.topic}`,
            `- Source: ${item.sourceLabel}`,
            `- Workflow score: ${item.workflowScore}`,
            `- Final score: ${item.finalScore}`,
            `- Loop: ${item.loopIterations} iterations / ${item.stopReason}`,
            ...item.optimizationNotes.map((note) => `- ${note}`),
            '',
            item.tweet,
            '',
          ]),
    ...
    `- Curiosity Gap: ${run.scores.curiosityGap}`,
    `- Emotional Charge: ${run.scores.emotionalCharge}`,
    `- Shareability: ${run.scores.shareability}`,
    `- Comment Bait: ${run.scores.commentBait}`,
    `- Screenshot Sentence: ${run.scores.screenshotSentence}`,
    '',
    `## Assets`,
